...
def init_db():
    try:
        ...
        cursor = conn.cursor()
        cursor.execute('''CREATE TABLE IF NOT EXISTS students (student_id TEXT PRIMARY KEY, name TEXT, face_encoding BLOB)''')
        cursor.execute('''CREATE TABLE IF NOT EXISTS attendance (id INTEGER PRIMARY KEY AUTOINCREMENT, student_id TEXT, name TEXT, date TEXT, time_in TEXT, time_out TEXT)''')
        conn.commit()
        conn.close()
...
def mark_attendance(std_id, name):
    ...
    cursor.execute("SELECT id, time_in, time_out FROM attendance WHERE student_id = ? AND date = ?", (std_id, today_date))
    record = cursor.fetchone()
    
    status_message = ""
    if record is None:
        cursor.execute("INSERT INTO attendance (student_id, name, date, time_in) VALUES (?, ?, ?, ?)", (std_id, name, today_date, current_time_str))
        conn.commit()
        ...
        if time_out is None:
            if isinstance(time_in, str):
                last_scan_time = datetime.combine(now.date(), datetime.strptime(time_in, "%H:%M:%S").time())
            else:
                last_scan_time = datetime.combine(now.date(), time_in)

            if now - last_scan_time > timedelta(minutes=1):
                cursor.execute("UPDATE attendance SET time_out = ? WHERE id = ?", (current_time_str, att_id))
                conn.commit()
            ...
            with st.spinner("Processing face data..."):
                try:
                    ...
                    if len(face_encodings) == 0:
                        ...
                        conn = sqlite3.connect(DB_PATH)
                        cursor = conn.cursor()
                        cursor.execute("INSERT OR REPLACE INTO students (student_id, name, face_encoding) VALUES (?, ?, ?)", (student_id, student_name, encoding_bytes))
                        conn.commit()
                        conn.close()
...
# ==========================================
    ...
    if len(known_encodings) == 0:
        st.warning("⚠️ No students found in the database. Please register students first.")
    else:
        ...
        def video_frame_callback(frame: av.VideoFrame) -> av.VideoFrame:
            ...
            if qr_data and qr_data in known_ids:
                ...
                cv2.rectangle(img, (50, 50), (img.shape[1]-50, img.shape[0]-50), (255, 255, 0), 4)
                cv2.putText(img, f"QR: {name}", (60, 90), cv2.FONT_HERSHEY_SIMPLEX, 0.8, (255, 255, 0), 2)
                cv2.putText(img, status, (60, 130), cv2.FONT_HERSHEY_SIMPLEX, 0.8, (255, 255, 0), 2)
                    ...
                    for ((top, right, bottom, left), face_encoding, face_landmarks) in zip(
                        ...
                        if True in matches:
                            ...
                            if avg_ear < 0.22:
                                ...
                                cv2.putText(img, f"{name}: {status}", (left, bottom + 25), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 255, 0), 2)
                            else:
                                cv2.rectangle(img, (left, top), (right, bottom), (0, 165, 255), 2)
                                cv2.putText(img, f"{name}: BLINK TO VERIFY", (left, bottom + 25), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 165, 255), 2)
                        else:
                            cv2.rectangle(img, (left, top), (right, bottom), (0, 0, 255), 2)
                            cv2.putText(img, "Unknown", (left, bottom + 25), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 0, 255), 2)
...
# ==========================================
    ...
    def fetch_attendance(selected_date=None):
        try:
            ...
            if not df.empty:
                df['time_in'] = df['time_in'].apply(lambda val: "" if pd.isnull(val) else str(val).split()[-1])
                df['time_out'] = df['time_out'].apply(lambda val: "" if pd.isnull(val) else str(val).split()[-1])
            return df
    ...
    with col2:
        ...
        if df.empty:
            ...
            st.download_button("📥 Download Data as CSV", data=csv, file_name=f"Attendance_{date_str}.csv", mime="text/csv")
